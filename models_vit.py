...
class ImageAdaptor(nn.Module):
    def __init__(
        ...
        self.pos_embed = nn.Parameter(
            scale * torch.randn(bucket_size**2 + 1, embed_dim)
        )
    ...
    def get_embed_positions(self, src_images):
        ...
        if window_size**2 > pos_embed.size(0):
            ...
            pos_embed = F.interpolate(
                old_pos_embed, size=(self.bucket_size, self.bucket_size), mode="bicubic"
            )
...
class TransformerEncoderLayer(nn.Module):
    def __init__(
        ...
        self.gamma_1 = nn.Parameter(
            layer_scale_init_value * torch.ones((self.embed_dim))
        )
    ...
    def forward(self, x: Tensor, attn_bias: Optional[Tensor] = None):
        ...
        if self.rp_bias:
            _, B = x.shape[:2]
            attn_bias = (
                self.get_rel_pos_bias().unsqueeze(0).expand(B, -1, -1, -1).flatten(0, 1)
            )
...
class TransformerEncoder(nn.Module):
    def __init__(
        ...
        self.layer_norm = (
            nn.LayerNorm(embed_dim) if not self.global_pool else nn.Identity()
        )
    ...
    def forward(self, image_info):
        ...
        if attn_bias is not None:
            attn_bias = (
                attn_bias.unsqueeze(0).expand(x.size(0), -1, -1, -1).flatten(0, 1)
            )
